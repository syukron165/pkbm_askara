  ...
  try {
    ...
    for (const row of data) {
      try {
        ...
        const nik = row.nik || row["NIK"] || row["NIP"];
        const positionApplied = row.positionApplied || row["Jabatan"] || "Staf Administrasi";
        const department = row.department || row["Divisi"] || "Tata Usaha & HRD";
        ...
        // 2. Create PublicRegistration (MANAJEMEN)
        const now = new Date();
        const yearMonth = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}`;
        const regCount = await db.publicRegistration.count({ where: { type: "MANAJEMEN" } });
        const seq = String(regCount + 1).padStart(4, "0");
        const registrationNumber = `REG-MNG-${yearMonth}-${seq}`;
        ...
        await db.publicRegistration.create({
          data: {
            ...
            positionApplied: positionApplied || "Staf",
    ...
    console.error("[POST /api/management/bulk] error:", error);

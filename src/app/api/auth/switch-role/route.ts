...
export async function POST(req: NextRequest) {
  try {
    ...
    // If user is super_admin, they can switch to any role
    // If user is admin or pendidik, they can switch between admin and pendidik
    const userRoles: Role[] = user.roles || [user.role];
    if (user.role === "super_admin" || user.role === "admin" || user.role === "pendidik") {
      if (!userRoles.includes("admin")) userRoles.push("admin");
      if (!userRoles.includes("pendidik")) userRoles.push("pendidik");
    }

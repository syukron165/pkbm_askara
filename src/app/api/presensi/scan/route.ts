...
export async function POST(req: NextRequest) {
  try {
    ...
    const now = new Date();
    const timeFormatted = `${now.getHours().toString().padStart(2, "0")}:${now.getMinutes().toString().padStart(2, "0")} WIB`;
    ...
    if (rawQR.startsWith("ASKARA-STUDENT:") || user.role === "pendidik" || user.role === "admin") {
      ...
      if (rawQR.startsWith("ASKARA-STUDENT:")) {
        ...
        if (parts.length >= 3) {
          targetStudentId = parts[1] || targetStudentId;
          targetNIS = parts[2] || targetNIS;
          if (parts[3]) targetStudentName = decodeURIComponent(parts[3]);
          if (parts[4]) targetClass = decodeURIComponent(parts[4]);
        }
      }
    ...
    const result = attendanceStore.recordAttendance(targetSessionCode, {
      studentId: user.id || studentId || "std-current",
      studentName: studentName || user.name || "Budi Santoso",
      nis: nis || "2025.10.048",
      className: className || "Paket C - Kelas X Merdeka",
      checkInTime: timeFormatted,
      method: "SCAN_QR_GURU",
      status: "HADIR",
    ...
    if (result.alreadyExists) {
      return NextResponse.json({
        success: true,
        alreadyCheckedIn: true,
        message: `Anda sudah tercatat hadir pada sesi ${session.title} (Check-in: ${result.record?.checkInTime}).`,
    ...
    return NextResponse.json({
      success: true,
      message: `Presensi Berhasil! Anda terverifikasi hadir di ${session.title}`,
      method: "SCAN_QR_GURU",
      sessionTitle: session.title,
      sessionType: session.type,
      teacherName: session.teacherName,

...
const AVAILABLE_ROLES = [
  { id: "pendidik", label: "Pendidik / Tutor Guru", icon: Users, color: "text-emerald-700 bg-emerald-50 border-emerald-200" },
  { id: "orang_tua", label: "Orang Tua / Wali", icon: HeartHandshake, color: "text-amber-700 bg-amber-50 border-amber-200" },
  { id: "admin", label: "Administrator & Staf", icon: Building, color: "text-indigo-700 bg-indigo-50 border-indigo-200" },
  { id: "bendahara", label: "Bendahara Keuangan", icon: Wallet, color: "text-teal-700 bg-teal-50 border-teal-200" },
  { id: "siswa", label: "Peserta Didik (Siswa)", icon: GraduationCap, color: "text-blue-700 bg-blue-50 border-blue-200" },
  { id: "super_admin", label: "Super Admin", icon: Shield, color: "text-purple-700 bg-purple-50 border-purple-200" },
];
...
export default function AdminUsersPage() {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (addForm.roles.length === 0) {
      showToast("Pilih minimal satu hak akses peran (role)", "error");
      return;
    }
  ...
  const handleOpenEdit = (user: UserItem) => {
    ...
    setEditForm({
      name: user.name,
      email: user.email,
      roles: parsedRoles.length > 0 ? parsedRoles : ["siswa"],
  ...
  const getRoleBadge = (roleStr: string) => {
    ...
    if (roles.length > 1) {
      return {
        label: `Ganda (${roles.length} Peran)`,
        bg: "bg-purple-100 text-purple-900 border-purple-200",
        icon: Sparkles,
      };
    }

    const r = roles[0] || "siswa";
    switch (r) {
      case "super_admin":
        return { label: "Super Admin", bg: "bg-purple-100 text-purple-900 border-purple-200", icon: Shield };
      case "admin":
        return { label: "Administrator", bg: "bg-blue-100 text-blue-900 border-blue-200", icon: Building };
      case "bendahara":
        return { label: "Bendahara", bg: "bg-teal-100 text-teal-900 border-teal-200", icon: Wallet };
      case "pendidik":
      case "guru":
      case "tutor":
        return { label: "Pendidik / Tutor", bg: "bg-emerald-100 text-emerald-900 border-emerald-200", icon: Users };
      case "siswa":
        return { label: "Peserta Didik", bg: "bg-indigo-100 text-indigo-900 border-indigo-200", icon: GraduationCap };
      case "orang_tua":
      case "orangtua":
        return { label: "Orang Tua / Wali", bg: "bg-amber-100 text-amber-900 border-amber-200", icon: HeartHandshake };
      default:
        return { label: r.toUpperCase(), bg: "bg-slate-100 text-slate-800 border-slate-200", icon: User };
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {toast && (
        <div
          className={`fixed bottom-5 right-5 z-50 px-4 py-3 rounded-2xl shadow-xl border flex items-center gap-2.5 text-xs font-bold transition-all duration-300 animate-slide-up ${toast.type === "success"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5">
        ...
        <div
          ...
          className={`p-4 rounded-2xl border transition-all cursor-pointer ${activeTab === "siswa"
        ...
        <div
          ...
          className={`p-4 rounded-2xl border transition-all cursor-pointer ${activeTab === "tutor"
        ...
        <div
          ...
          className={`p-4 rounded-2xl border transition-all cursor-pointer ${activeTab === "orang_tua"
        ...
        <div
          ...
          className={`p-4 rounded-2xl border transition-all cursor-pointer ${activeTab === "manajemen"
        ...
        <div className="border-b border-slate-200/80 px-4 pt-3 flex flex-wrap items-center justify-between gap-3 bg-slate-50/50">
          <div className="flex items-center gap-1 overflow-x-auto pb-2 scrollbar-none">
            ...
            <button
              type="button"
              onClick={() => setActiveTab("siswa")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${activeTab === "siswa"
            ...
            <button
              type="button"
              onClick={() => setActiveTab("orang_tua")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${activeTab === "orang_tua"
            ...
            <button
              type="button"
              onClick={() => setActiveTab("tutor")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${activeTab === "tutor"
                  ? "bg-emerald-700 text-white shadow-sm"
                  : "bg-transparent text-slate-600 hover:bg-slate-100 hover:text-slate-900"
                }`}
            >
            ...
            <button
              type="button"
              onClick={() => setActiveTab("manajemen")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${activeTab === "manajemen"
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            <div className="inline-flex p-1 bg-slate-100 rounded-xl border border-slate-200 text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold transition ${statusFilter === "all"
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold transition ${statusFilter === "active"
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold transition ${statusFilter === "inactive"
                    ...
                    <tr key={u.id} className="hover:bg-slate-50/80 transition">
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-bold transition shadow-2xs border ${u.isActive
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${u.isActive ? "bg-emerald-500 animate-pulse" : "bg-slate-400"
                              }`}
                          />
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 p-6 sm:p-7 space-y-5">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                ...
                <div className="sm:col-span-2 space-y-2 rounded-2xl border border-slate-200 bg-slate-50/80 p-3.5">
                  <label className="block font-bold text-slate-800 text-xs">
                    Hak Akses Peran (Multi-Role Support) <span className="text-rose-500">*</span>
                  </label>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    {AVAILABLE_ROLES.map((roleOption) => {
                      const isChecked = addForm.roles.includes(roleOption.id);
                      const Icon = roleOption.icon;
                      return (
                        <label
                          key={roleOption.id}
                          className={`flex items-center gap-2 p-2 rounded-xl border transition cursor-pointer ${isChecked
                              ? `${roleOption.color} font-bold shadow-2xs`
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setAddForm({ ...addForm, roles: [...addForm.roles, roleOption.id] });
                              } else {
                                setAddForm({ ...addForm, roles: addForm.roles.filter((r) => r !== roleOption.id) });
                              }
                            }}
                  ...
                  <p className="text-[10px] text-slate-500 italic mt-1">
                    * Pengguna dengan lebih dari 1 peran akan mendapatkan fitur **Switch Role** di Header secara otomatis.
                  </p>
                </div>
                ...
                {addForm.roles.includes("orang_tua") && (
                  <>
                    <div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 p-6 sm:p-7 space-y-5">
            ...
            <form onSubmit={handleSubmitEdit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                ...
                {/* Multi-Role Checkbox Selector in Edit Form */}
                <div className="sm:col-span-2 space-y-2 rounded-2xl border border-slate-200 bg-slate-50/80 p-3.5">
                  <label className="block font-bold text-slate-800 text-xs">
                    Hak Akses Peran (Multi-Role Support) <span className="text-rose-500">*</span>
                  </label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    {AVAILABLE_ROLES.map((roleOption) => {
                      const isChecked = editForm.roles.includes(roleOption.id);
                      const Icon = roleOption.icon;
                      return (
                        <label
                          key={roleOption.id}
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setEditForm({ ...editForm, roles: [...editForm.roles, roleOption.id] });
                              } else {
                                setEditForm({ ...editForm, roles: editForm.roles.filter((r) => r !== roleOption.id) });
                              }
                            }}
                            className="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"
                          />
